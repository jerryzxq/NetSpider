...
{
    public class suzhouyuanqu : ISocialSecurityCrawler
    {
        #region 公共变量
        ...
        string socialCity = "js_suzhouyuanqu";
        #endregion
...
return RSAUtils.encryptedString(RSAUtils.getKeyPair(t1,t2,t3), p2); 
                ...
                ob[0] = "010001";
                ob[1] = socialReq.Username;
                ob[2] = httpResult.Html.Replace(":010001","");
                ob[3] = CommonFun.GetTimeStamp() + ":" + socialReq.Password;
                ...
                postdata = string.Format("uname={0}&upass={1}&sessionid={2}&identify={3}&answer={4}&param3={5}", socialReq.Username, CommonFun.GetMd5Str(socialReq.Password), cookies["JSESSIONID"].Value, socialReq.Vercode, answer, raspassword);
                httpItem = new HttpItem()
                {
                    URL = Url,
                    Method = "POST",
                ...
                Res.BirthDate = baseinfo.EMP_BIRTHDAY;
                ...
                Res.ZipCode = baseinfo.EMP_POST;
                Res.Address = baseinfo.EMP_ADDRESS;
                #endregion

                #region 第三步，明细
                Url = baseUrl + "account/accountQuery?regkey=" + regkey + "&itype=1"; ;
                httpItem = new HttpItem()
                {
                    URL = Url,
                    Method = "post",
                ...
                {
                    if (!_detail.renderType.Contains("正常工资") && !_detail.renderType.Contains("补交工资"))
                    {
                        continue;
                    }
                    ...
                    detailRes.SocialInsuranceTime = _detail.belongMonth;
                    if (_detail.renderType.Contains("正常工资"))
                    {
                    ...
                    detailRes.SocialInsuranceBase = _detail.baseMoney.ToDecimal(0);
                    detailRes.PensionAmount = _detail.commonRetiredAcc.ToDecimal(0);
                    detailRes.MedicalAmount = _detail.retiredMedicalAcc.ToDecimal(0);
                ...
                while (startdate < DateTime.Now)
                {
                    ...
                    foreach (string item in results)
                    {
                        ...
                        detailRes = null;
                        string SocialInsuranceTime = tdrow[2].Replace("缴交", "").Replace("补缴", "").ToInt(0).ToString();
                        if (SocialInsuranceTime != "0")
                        {
                        ...
                        if (detailRes == null)
                        {
                            ...
                            {
                                detailRes.SocialInsuranceTime = tdrow[2].Replace("缴交", "");
                                detailRes.PaymentFlag = ServiceConsts.SocialSecurity_PaymentFlag_Normal;
                                detailRes.PaymentType = ServiceConsts.SocialSecurity_PaymentType_Normal;
                            }
                            else if (tdrow[2].StartsWith("补缴"))
                            {
                                ...
                                {
                                    detailRes.SocialInsuranceTime = tdrow[2].Replace("补缴", "");
                                    detailRes.PaymentFlag = "补缴";
                                    detailRes.PaymentType = ServiceConsts.SocialSecurity_PaymentType_Normal;
                                }
                            }
                            ...
                            detailRes.CompanyPensionAmount = tdrow[11].ToDecimal(0);
                            detailRes.CompanyMedicalAmount = tdrow[12].ToDecimal(0);
                            detailRes.EmploymentInjuryAmount = tdrow[13].ToDecimal(0);
                            detailRes.UnemployAmount = tdrow[14].ToDecimal(0);
                            detailRes.MaternityAmount = tdrow[15].ToDecimal(0);
                            detailRes.IllnessMedicalAmount = tdrow[16].ToDecimal(0);
