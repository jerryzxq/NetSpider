...
{
    public class yinzhouqu : IProvidentFundCrawler
    {
        #region 公共变量
        ...
        string baseUrl = "http://yzgjj.nbyz.gov.cn/";
        string fundCity = "zj_yinzhouqu";
        #endregion
        ...
        public Entity.Service.ProvidentFundQueryRes ProvidentFundQuery(ProvidentFundReq fundReq)
        {
            ...
            try
            {
                ...
                Res.SalaryBase = CommonFun.GetMidStr(httpResult.Html, "本人工资基数:", "元").ToDecimal(0);
                Res.PersonalMonthPayRate = CommonFun.GetMidStr(httpResult.Html, "个人缴存比例:", "，").ToDecimal(0);
                Res.CompanyMonthPayRate = CommonFun.GetMidStr(httpResult.Html, "单位缴存比例:", "</td>").ToDecimal(0);
                ...
                foreach (string s in results)
                {
                    ...
                    {
                        detail.PersonalPayAmount = tdRow[2].ToDecimal(0) * perAccounting;
                        detail.CompanyPayAmount = tdRow[2].ToDecimal(0) * (1 - perAccounting);
