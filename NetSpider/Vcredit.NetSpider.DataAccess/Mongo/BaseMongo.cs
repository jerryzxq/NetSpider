...
namespace Vcredit.NetSpider.DataAccess.Mongo
{
    ...
    public class BaseMongo
    {
        ...
        /// 默认连接config配置
        /// </summary>
        private readonly string DefaultConnectConfig = "MongoDB";
        /// <summary>
        /// 默认数据库
        /// </summary>
        private const string DefaultMongoDatabase = "MongoDB";

        public BaseMongo(string mongoDatabase, string connectConfig)
        {
            if (!string.IsNullOrWhiteSpace(connectConfig))
                DefaultConnectConfig = connectConfig;
            this._db = GetDataBase(mongoDatabase);
        }

        public BaseMongo(string mongoDatabase = DefaultMongoDatabase)
        {
            this._db = GetDataBase(mongoDatabase);
        }

        public BaseMongo(MongoDatabase db)
        {
            this._db = db;
        }

        private MongoDatabase GetDataBase(string mongoDatabase)
        {
            MongoDatabase db = null;
            var config = ConfigurationManager.ConnectionStrings[DefaultConnectConfig];
            if (config != null)
            {
                //创建Mongo的客户端
                MongoClient client = new MongoClient(config.ToString());
                //得到服务器端并且生成数据库实例
                //db = client.GetServer().GetDatabase(mongoDatabase);
                ...
                if (mongoServer.Settings.DefaultCredentials != null)
                {
                    ...
                    db = mongoServer.GetDatabase(mongoDatabase, credentials);
                }
                else
                {
                    db = mongoServer.GetDatabase(mongoDatabase);
                }
            }
            return db;
        }
        ...
        /// <typeparam name="T">移除的数据类型</typeparam>
        /// <param name="collectionName">指定的集合名词</param>
        public void Drop(string serverName)
        {
            MongoDatabase mdb = this.GetDataBase(serverName);
            //根据指定查询移除数据
            mdb.Drop();
        }
