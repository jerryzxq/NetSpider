...
namespace Vcredit.Common.Helper
{
    ...
    internal class HttpHelper_2
    {
        ...
        private void GetData(HttpItem item, HttpResult result)
        {
            ...
            {
                requestcookies = request.CookieContainer.GetCookies(request.RequestUri);
            }
            ...
            for (int i = 0; i < requestcookies.Count; i++)
            {
                if (cookies.Count > 0)
                {
                    ...
                    {
                        if (cookies[j].Name != requestcookies[i].Name)
                        {
                            cookies.Add(requestcookies[i]);
                        }
                    }
                }
                else
            ...
            if (cookies.Count > 0)
            {
                result.CookieCollection = cookies;
            }
            #endregion
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            if (item.SecurityProtocolType != null)
            {
                ServicePointManager.SecurityProtocol = (SecurityProtocolType)item.SecurityProtocolType;
            }
        }
        /// <summary>
        /// 设置证书
