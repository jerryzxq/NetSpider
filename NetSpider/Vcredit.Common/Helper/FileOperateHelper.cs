...
namespace Vcredit.Common.Helper
{
    public class FileOperateHelper
    {
        ...
        *****************************************/
        ...
        #region 保存图片到本地
        public static void WriteVerCodeImage(string ImageName, byte[] bytes)
        {
            string path = string.Empty;
            if (HttpContext.Current != null)
            {
                path = HttpContext.Current.Server.MapPath(@"/files/vercode/");
            }
            else
            {
                path = "C://";
            }
            System.IO.File.WriteAllBytes(path + ImageName + ".jpg", bytes);
