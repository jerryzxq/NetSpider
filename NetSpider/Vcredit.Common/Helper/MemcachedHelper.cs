...
namespace Vcredit.Common.Helper
{
    public class MemcachedHelper
    {
        ...
        {
            string memcachedServerString = AppSettings.memcachedServer;
            if (!string.IsNullOrEmpty(memcachedServerString))
            {
                var memcachedServers = memcachedServerString.Split(',');
                for (int i = 1; i <= memcachedServers.Length; i++)
                {
                    Servers.Add("Svr" + i, memcachedServers[i-1]);
                }
            }
        }
