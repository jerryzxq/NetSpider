...
namespace Dapper
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        public static IList<T> GetList<T>(this IDbConnection connection, string sqlWhere = null, IDictionary<string, object> param = null, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            string sql;
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
            ...
            if (param != null)
            {
                ...
                {
                    dynParms.Add(kvp.Key, kvp.Value);
                }
            }

            IList<T> obj = null;

            obj = connection.Query<T>(sql, dynParms, transaction: transaction, commandTimeout: commandTimeout).ToList();
