...
namespace Vcredit.NetSpider.Monitor.job
{
    ...
    class MobileColmonJob : IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            applyList = logMongo.GetColmonExcLog("mobile", staData, endData);
            ...
            if (applyList.Count > 0)
            {
                ...
                foreach (var item in groups)
                {
                    monitorlogMongo.SaveMonitorLog(new MonitorLog()
                    {
                        ...
                        Type = "Colmon",
                        Description = Common.GetWebsiteName(item.Key) + "必要信息异常，异常条数：" + item.NumProducts
                    });
                    CommonFun.SendMail("程序异常提醒", Common.GetWebsiteName(item.Key) + "信息异常");
                    Console.WriteLine(Common.GetWebsiteName(item.Key) + "信息异常，异常条数：" + item.NumProducts + "，时间：" + DateTime.Now);
                }
            }
            else
                Console.WriteLine("信息无异常，时间：" + DateTime.Now);
