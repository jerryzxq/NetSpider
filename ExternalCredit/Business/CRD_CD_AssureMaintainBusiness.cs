...
namespace Vcredit.ExtTrade.BusinessLayer
{
    ...
    /// 担保征信上报信息
    /// </summary>
    public class CRD_CD_AssureMaintainBusiness
    {
        ...
        public void UpLoadingMarkDefault(int diffHour)
        {
            ...
            var entities = dao.Select<CRD_CD_AssureMaintainEntity>(x => x.State == (int)SysEnums.AssureReportState.UpLoading && 
                                                                            x.UpdateTime < updateDateTime
                                                                            )
                                                                  .ToList();

            var msg = string.Format("担保前维护（清贷非清贷） --> 登陆失败导致状态为等待上传的总数为：{0}条，修改状态为默认值，等待重新上传", entities.Count);
