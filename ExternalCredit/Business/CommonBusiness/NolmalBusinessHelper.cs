...
{
    public class NolmalBusinesshelper
    {
        ...
        {
            if (lndList == null || lndList.Count==0)
            {
                return;
            }
            long lndid = 0;
        ...
        {
            if (stnList == null||stnList.Count==0)
                return;
            long stnid = 0;
        ...
        public void MoveCommitExcel(string orginalName)
        {
            string newfile = orginalName.Replace("Return", "Returned");
            string newdiretory = newfile.Substring(0, newfile.LastIndexOf('\\'));
            if (!Directory.Exists(newdiretory))
            {
                Directory.CreateDirectory(newdiretory);
            }
            FileOperateHelper.FileMove(orginalName, newfile);
        }
    }
}
