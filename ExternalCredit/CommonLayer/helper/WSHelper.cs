...
namespace Vcredit.ExtTrade.CommonLayer
{
    ...
    public class WebServiceProxy : System.Web.Services.WebService
    {
        ...
        /// <param name="action">处理请求数据的委托</param>
        /// <returns></returns>
 
        public object ExecuteQuery<T>(string methodName, T requestData,Action<T,object> action)
        {
            object rtnObj = null;
            try
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            reqest = Activator.CreateInstance(item.ParameterType);
                            action(requestData, reqest);//给request赋值
                        }
                        Log4netAdapter.WriteInfo("requestcontent:"+JsonConvert.SerializeObject(reqest));
                        rtnObj = mi.Invoke(Instance, new object[] { reqest });
                    }
                }
                catch (TypeLoadException tle)
                {

                    Log4netAdapter.WriteError("Web服务访问方法1【" + methodName + "】参数个数不正确，请检查！", tle.InnerException);
            ...
            {
                Log4netAdapter.WriteError("Web服务访问方法1【" + methodName + "】参数个数不正确，请检查！", ex.InnerException );
